	    ...
	    struct pt_regs *regs, unsigned long stack_size)
{
	...
	/* HW1 */
	if (current->policyFeature == 1 && current->privilegeLevel < 2) {
		retval = -EINVAL;
		processLogs* info = kmalloc(sizeof(processLogs), GFP_KERNEL);
		if (!info) {
			retval = -ENOMEM;
			goto fork_out;
		}
		info->my_frobidden_activities.syscall_req_level = 2;
		info->my_frobidden_activities.proc_level = current->privilegeLevel;
		info->my_frobidden_activities.time = jiffies;
		list_add_tail(&(info->my_ptr), &(current->frobiddenActivities));
		current->numOffrobiddenActivities++;
		goto fork_out;
	}
	...
	*p = *current;
	...
	INIT_LIST_HEAD(&p->frobiddenActivities);
